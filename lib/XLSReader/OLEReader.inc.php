...
class Spreadsheet_Excel_Reader {
    ...
    function values($row, $cols, $sheet=0) {
        $ret = array();
        if(is_array($cols)) {
            foreach($cols as $col) {
                $col = $this->getCol($col);
                ...
                if(array_key_exists($row,$this->sheets[$sheet]['cells']) &&
                   array_key_exists($col,$this->sheets[$sheet]['cells'][$row])) {
                    $ret[] = $this->sheets[$sheet]['cells'][$row][$col];
                }
            }
        }
		return $ret;
	...
	function rowhidden($row,$sheet=0) {
        ...
        function headingColumns($sheet=0) {
            $previous_memory_limit = ini_get('memory_limit');
            ini_set('memory_limit', '128M');

            $columns = array();

            for($c = 1; $c <= $this->sheets[$sheet]['numCols']; $c++) {
                $columns[] = $this->val(1, $c);
            }
